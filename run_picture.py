...
def train_picture(model, optimizer, train_dataloader, valid_dataloader, device):  
    ...
    np.savetxt('./result/train_picture_loss.txt', train_losses)
    train_accuracy = np.array(train_accuracy)
    np.savetxt('./result/train_picture_accuracy.txt', train_accuracy)
    valid_losses = np.array(valid_losses)
    np.savetxt('./result/valid_picture_loss.txt', valid_losses)
    valid_accuracy = np.array(valid_accuracy)
    np.savetxt('./result/valid_picture_accuracy.txt', valid_accuracy)

def run_picture(LR, momentum, train_dataloader, valid_dataloader, test_dataloader):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        model = MutilModalClassifier('Picture_model')
        if torch.cuda.is_available():
            model.cuda() 
            print("GPU is using")
        optimizer = optim.SGD(model.parameters(), lr=LR, momentum=momentum)
        train_picture(model, optimizer, train_dataloader, valid_dataloader, device)